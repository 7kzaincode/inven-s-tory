...
const AddItem: React.FC<AddItemProps> = ({ ownerId }) => {
  ...
  const handleSaveItem = async () => {
    ...
    try {
      ...
      // ORGANIZED STORAGE PATH: archives/[userId]/[itemName]/[timestamp].png
    ...
    <div className="flex flex-col items-center w-full max-w-xl mx-auto py-12 px-4">
      ...
      {step === 'upload' && (
        <div className="w-full flex flex-col items-center space-y-12">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="w-full aspect-square border-2 border-dashed border-zinc-100 bg-[#FDFDFD] cursor-pointer hover:border-zinc-900 transition-all flex flex-col items-center justify-center">
            <span className="text-[40px] font-light text-zinc-200">+</span>
            <span className="text-[12px] uppercase tracking-widest font-bold text-zinc-400">Source Asset</span>
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*" />
          </div>
          <button onClick={() => setMode('select')} className="text-[10px] uppercase tracking-widest text-zinc-300 font-bold">Return</button>
        </div>
      )}

      {step === 'protocol' && (
        <div className="w-full space-y-12">
          <div className="p-10 border border-zinc-900 bg-zinc-900 text-white space-y-6">
          ...
          <div className="flex items-center gap-6 p-6 border border-zinc-100 bg-zinc-50 cursor-pointer" onClick={() => setComplianceAccepted(!complianceAccepted)}>
             <div className={`w-6 h-6 border-2 flex items-center justify-center ${complianceAccepted ? 'bg-zinc-900 border-zinc-900' : 'bg-white border-zinc-200'}`}>
          ...
          <button onClick={() => setStep('finalize')} disabled={!complianceAccepted} className="w-full py-7 bg-zinc-900 text-white text-[11px] font-bold uppercase tracking-[0.4em] disabled:opacity-20">PROCEED</button>
        </div>
      )}

      {step === 'finalize' && (
        <div className="w-full space-y-12">
          <div className="aspect-square bg-white border border-zinc-100 flex items-center justify-center p-12">
          ...
          <div className="space-y-10">
            ...
            <div className="flex gap-12 items-center">
              <div className="flex items-center gap-4 cursor-pointer" onClick={() => setIsForSale(!isForSale)}>
                <div className={`w-5 h-5 border ${isForSale ? 'bg-zinc-900' : ''}`} />
                <label className="text-[10px] font-bold uppercase tracking-widest">Sale</label>
              </div>
              {isForSale && <input type="number" value={price} onChange={e => setPrice(Number(e.target.value))} className="flex-1 border-b border-zinc-900 text-[14px] font-bold outline-none text-right" placeholder="$" />}
            </div>
            <button onClick={handleSaveItem} disabled={loading || !name} className="w-full py-7 bg-zinc-900 text-white text-[11px] font-bold uppercase tracking-[0.4em]">
