  ...
  const fetchArchives = async (query: string = searchQuery) => {
    ...
    try {
      ...
      if (profiles) {
        ...
        // Filter for identities with at least one public item or active search
    ...
    <div className="w-full flex flex-col items-center space-y-24">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <section className="lg:col-span-8 space-y-12">
          <h3 className="text-[11px] uppercase tracking-[0.3em] font-bold text-zinc-400 border-b border-zinc-50 pb-4">RECENTLY ACTIVE IDENTITIES</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {loading && <div className="col-span-full text-center py-20 text-[11px] uppercase tracking-widest text-zinc-400 font-bold animate-pulse">Syncing Directory...</div>}
            {!loading && archives.length === 0 && <div className="col-span-full text-center py-20 text-[11px] uppercase tracking-widest text-zinc-300 font-bold">No archives found.</div>}
          </div>
        </section>
            ...
            {tradeAds.map(ad => (
              <div key={ad.id} className="bg-white border border-zinc-100 p-8 shadow-sm hover:shadow-md transition-shadow group">
                ...
                <div className="flex justify-between items-center pt-6 border-t border-zinc-50">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">LF: {ad.looking_for}</span>
                  <div className="flex gap-6">
                    <Link to={`/messages/${ad.owner_id}`} className="text-[10px] font-bold uppercase tracking-widest text-zinc-900 underline decoration-zinc-200 underline-offset-4 hover:decoration-zinc-900 transition-all">Message</Link>
                    <Link to={`/trade/${ad.owner?.username}`} className="text-[10px] font-bold uppercase tracking-widest text-zinc-900 border border-zinc-900 px-5 py-2 hover:bg-zinc-900 hover:text-white transition-all">Offer</Link>
                  </div>
                </div>
              </div>
            ))}
            <Link to="/trade-ads" className="block text-center py-6 bg-zinc-900 text-white text-[11px] uppercase tracking-[0.4em] font-bold hover:bg-black transition-all">POST BULLETIN</Link>
          </div>
        </section>
      </div>
    </div>
